<div>
  <h2 class="mb-2 font-bold">{{ __('Marital statuses') }}</h2>

  <div class="mb-4 flex items-center justify-between">
    <p class="text-sm text-gray-500">{{ __('You can add, edit and delete marital statuses.') }}</p>

    @if (! $addMode)
      <x-button.secondary wire:click="toggleAddMode">
        <span>{{ __('New marital status') }}</span>
      </x-button.secondary>
    @endif
  </div>
  ...
  <div>
    @if ($addMode)
      <form wire:submit="store" class="mb-4 space-y-5 rounded-lg border border-gray-200 p-4">
        <!-- name -->
        <div class="relative">
          <x-input-label for="name" :value="__('Marital status name')" />
  ...
  <div class="mb-4 rounded-lg border border-gray-200">
    @forelse ($maritalStatuses as $maritalStatus)
      <div class="flex border-b border-b-gray-200 py-2 last:border-b-0 hover:bg-blue-50 first:hover:rounded-t-lg last:hover:rounded-b-lg sm:flex-row sm:items-center sm:justify-between sm:px-3 dark:hover:bg-gray-600">
        @if ($editedMaritalStatusId !== $maritalStatus['id'])
        ...
        @else
          <form wire:submit="update({{ $maritalStatus['id'] }})" class="space-y-5">
            <!-- name -->
            <div class="relative">
              <x-input-label for="name" :value="__('Marital status name')" />
    ...
    @empty
      <div id="blank-state" class="flex flex-col items-center rounded-lg bg-white p-6">
        <div class="mb-5 rounded-full bg-slate-100 p-4">
          <x-lucide-heart class="h-6 w-6 text-gray-500" />
        </div>

        <p class="text-center">{{ __('Marital statuses are used to express the marital status of contacts.') }}</p>
      </div>
    @endforelse
  </div>
</div>
