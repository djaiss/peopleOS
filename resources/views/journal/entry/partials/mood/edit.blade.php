...
<form id="edit-mood-{{ $mood->id }}" x-target="edit-mood-{{ $mood->id }}" x-target.back="edit-mood-{{ $mood->id }}" action="{{ route('journal.entry.mood.update', ['year' => $year, 'month' => $month, 'day' => $day, 'mood' => $mood->id]) }}" method="POST" x-data="{ mood: {{ $mood->mood }} }" class="mx-auto max-w-xl rounded-lg border border-gray-200 bg-white text-center transition-all duration-800 ease-in-out">
  @csrf
  @method('PUT')
  ...
  <div class="flex items-center justify-between border-t border-gray-200 px-4 py-4">
    <x-button.secondary x-target="edit-mood-{{ $mood->id }}" href="{{ route('journal.entry.show', ['year' => $year, 'month' => $month, 'day' => $day]) }}">
      {{ __('Cancel') }}
    </x-button.secondary>
