  ...
  <div class="flex gap-4 rounded-lg border border-gray-200 bg-white">
    <div class="flex flex-col gap-4 border-r border-gray-200 p-4">
      ...
      <div class="flex items-baseline gap-2">
        <p class="text-2xl font-semibold">{{ $personSeenReports['currentYearCount'] }}</p>
        <p class="text-sm text-gray-500">{{ __('in :year', ['year' => date('Y')]) }}</p>
        @if ($personSeenReports['previousYearCount'] > 0)
          <span class="text-gray-400">·</span>
          <p class="text-sm text-gray-500">{{ $personSeenReports['previousYearCount'] }} {{ __('in :year', ['year' => date('Y') - 1]) }}</p>
        @endif
      </div>
    </div>
    <div class="p-4">
  ...
  <div x-cloak x-show="encountersExpanded" id="encounters-list" class="mt-4 rounded-lg border border-gray-200 bg-white">
    ...
    <div class="divide-y divide-gray-200">
      ...
      <div class="flex flex-col items-center justify-center py-8 px-4 text-center">
          ...
          <p class="text-sm font-medium text-gray-900 mb-1">{{ __('No encounters recorded yet') }}</p>
