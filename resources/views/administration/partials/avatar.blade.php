    ...
    <div
      ...
      <div x-ref="frontFace" class="absolute w-full border border-gray-200 bg-white [backface-visibility:hidden] sm:rounded-lg">
        <div class="grid grid-cols-3 items-center rounded-t-lg p-3 last:rounded-b-lg hover:bg-blue-50">
          <x-image width="58" height="58" class="col-span-2 h-16 w-16 rounded-full object-cover p-[0.1875rem] shadow-sm ring-1 ring-slate-900/10" src="{{ auth()->user()->getAvatar(64) }}" srcset="{{ auth()->user()->getAvatar(64) }} 1x, {{ auth()->user()->getAvatar(128) }} 2x" alt="{{ auth()->user()->name }}" />
