...
<form x-target="gender-{{ $gender['id'] }}" x-target.back="gender-{{ $gender['id'] }}" id="gender-{{ $gender['id'] }}" action="{{ route('administration.personalization.genders.update', $gender['id']) }}" method="POST" class="space-y-5 p-4 hover:bg-blue-50">
  @csrf
  @method('PUT')
